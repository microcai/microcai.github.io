...
avbot 5.0 系列第3个版本 5.1  [发布](http://sourceforge.net/projects/avbot/files/sources/avbot-5.1.tar.bz2/download)了，同时发布的还有　[rpm 包](https://sourceforge.net/projects/avbot/files/rpm/) 和 [windows 包](https://sourceforge.net/projects/avbot/files/win32/)

avbot 的版本规则是, 每月提升一个大版本号, 因为是开发 avbot 的第 6 个月了, 所以大版本号提升到 5 了.

avbot 5.1 发布, 修复了一个非常严重,困扰了我们很久的一个 100% CPU 占用问题. 
当然, 5.0 版本也顺带跟随腾讯更新了群图片的图床API. 更新avbot后群图片获取不到的bug就消失了.
